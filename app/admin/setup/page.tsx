...
import { CheckCircle, XCircle, RefreshCw, Database, Trash2 } from "lucide-react"
...
export default function SetupPage() {
  ...
  const envVarStatus = [
    { name: "MONGO_URL", status: healthStatus?.environmentVariables.mongoUrl, required: true },
    { name: "ADMIN_EMAIL", status: healthStatus?.environmentVariables.adminEmail, required: true },
    { name: "ADMIN_PASSWORD", status: healthStatus?.environmentVariables.adminPassword, required: true },
    { name: "NEXTAUTH_SECRET", status: healthStatus?.environmentVariables.nextAuthSecret, required: true },
    { name: "GROQ_API_KEY", status: healthStatus?.environmentVariables.groqApiKey, required: true },
    { name: "RESEND_API_KEY", status: healthStatus?.environmentVariables.resendApiKey, required: false },
  ]

  const allRequiredVarsSet = envVarStatus.filter((v) => v.required).every((v) => v.status)
    ...
    <div className="container mx-auto px-4 py-16">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Check your application configuration and environment variables</p>
        </div>
                ...
                {envVarStatus.map((envVar) => (
                  <div key={envVar.name} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={envVar.status ? "default" : "destructive"}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Initialize your portfolio with sample data or clear existing data.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Button onClick={seedDatabase} disabled={isSeeding || !allRequiredVarsSet}>
                  <Database className="h-4 w-4 mr-2" />
                  {isSeeding ? "Seeding..." : "Seed Database"}
                </Button>
                <Button onClick={clearDatabase} disabled={isClearing || !allRequiredVarsSet} variant="destructive">
              ...
              {!allRequiredVarsSet && (
                <p className="text-sm text-yellow-600">
                  ⚠️ Please ensure all required environment variables are set before managing the database.
                </p>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4 border-t">
                ...
                <div className="text-sm text-muted-foreground space-y-1">
                  <p>Email: {healthStatus?.environmentVariables.adminEmail ? "✓ Configured" : "❌ Not set"}</p>
                  <p>Password: {healthStatus?.environmentVariables.adminPassword ? "✓ Configured" : "❌ Not set"}</p>
                </div>
              </div>
